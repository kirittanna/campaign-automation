import { generate } from "@/lib/genai/image-replicate-inputs";
...
const generateHeroImages = async (data: Partial<CampaignBrief>) => {
    ...
    const heros = await Promise.all(OUTPUT.map(async ({ aspectRatio, prompt, size }) => {
        const result = await generate({
            ...
            providerOptions: {
                replicate: {
                    image_input: products?.map(p => p.image) as string[],
        ...
        console.log("Generated hero image with", aspectRatio, size);
